...
## 📌 未実装（提案から着手候補・優先度順）
- [ ] ランデブーモード（SRT 相当の UDP 同時オープン + STUN 公開アドレス発見 + 全測定の UDP 化）
      — 両側とも着信不可（家庭用 NAT 同士）の構成向け。CGNAT 同士は symmetric NAT が多く
      原理的に成功率が低いため、その場合は VPS 受信アンカーを推奨（docs/network_check.md 参照）
